...
<head>
    ...
    <title>Laporan Kas Nama Perusahaan</title>
...
<body>
    <div class="container mt-5">
        ...
        <div class="header text-center">
            ...
            <p class="mb-1">Laporan Kas Kecil <br> (Petty Cash)</p>
            
            @php
                // Pastikan hanya mengambil tanggal dari data yang ada
                if ($saldos->isNotEmpty()) {
                    $tanggalAwal = $saldos->min('tanggal_transaksi');
                    $tanggalAkhir = $saldos->max('tanggal_transaksi');
                } else {
                    $tanggalAwal = now();
                    $tanggalAkhir = now();
                }
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    @forelse($saldos as $saldo)
                        @foreach($saldo->saldo_items as $item)
                            @php
                                // Ambil status transaksi
                                $status = $saldo->transaction ? $saldo->transaction->status : null;
                                $lunas = $saldo->lunas;

                                // Tentukan apakah transaksi ini harus ditampilkan
                                if ($status === 'unpaid' && !$lunas) {
                                    continue; // Lewati transaksi unpaid
                                }

                                $isDebit = $saldo->kas_bank == 'Terima Dana';
                                $biaya = $item->biaya;
                            @endphp
                ...
                <tfoot>
                    <tr>
                        ...
                        <th>Rp. {{ number_format($saldo_awal + $totalDebit - $totalKredit, 2, ',', '.') }}</th>
                    </tr>
                </tfoot>
            </table>
        </div>
